...
public  void TestmatchingAlgo(String text,int c,Hashtable t, int patternLength){ 
      int pos=1, cur=pos,begin, end, terminate, j=0, matchwindow;
    //  System.out.println("text length:"+text.length());
     // for(int i=0;i<text.length();i++) System.out.println("pos:"+i+":"+text.charAt(i));
      while(j<=((text.length()-c))){
          ...
          while(matchwindow==1){
             ...
             if(t.containsKey(st)){ //เทียบแล้วมีในตาราง
                  ...
                  if(A!=null){
                     ...
                     // System.out.println("Match here."+text.charAt(end-1)+"pos:"+end);
                      System.out.println("Match here pos:"+end);
                  }                       
...
//กรณีลงตัวจำนวนความยาวเท่ากันพอดี  แก้ไขเมื่อ  14 มี.ค. 61
public  void TestmatchingAlgoX(String text,int c,Hashtable t, int patternLength){ 
      int pos=1, cur=pos,begin, end, terminate, j=0, matchwindow,matchnumber=0, mcount=0;
         ...
         // System.out.println("j="+j);
          ...
          while(matchwindow==1){
             ...
             if(t.containsKey(st)){ //เทียบแล้วมีในตาราง
                  ...
                  HashSet A=(HashSet)h.get(String.valueOf(pos)+":1");
                  HashSet B=(HashSet)h.get(String.valueOf(pos)+":0");
                 // System.out.println("pos:"+pos+ "  Before if Set"+" patternLength:"+patternLength);
                  if(pos==patternLength) matchwindow=0;
                  if(A!=null){
                      if(pos==1){
                      //  System.out.println("เทียบตรง:"+st+":"+h.get(st));
                      //  System.out.println("Match here."+text.charAt(end-1)+"position:"+end);
                      //  System.out.println("Match here position:"+end+" A:"+A+" pos:"+j);
                     //--------   System.out.println("Match here position:"+j);-----//
                        mcount++;
                      }else{
                        // System.out.println("Need intersection with B:"+B +" A:"+A);
                         A = interSection(A,C);
                         if(A.size()>0) {
                       //--------      System.out.println("Match here.pos:"+j); -----//
                             mcount++;
                         }
                      }
                  }                       
...
//                     System.out.println("เทียบตรง:"+st+":"+t.get(st));
                ...
                //        System.out.println(" First C:"+C);
                        ...
                        C = interSection(B,C);  //ต้องอินเทอร์เซคชัน
                 ...
                 // ภายใต containKey ที่ต้องหาต่อไปให้หมด
...
private HashSet interSection(HashSet A, HashSet B){
...
public void TestmatchingAlgo1(String text,int c,Hashtable t, int patternLength, int r[], int max){ 
      int pos=1, cur=pos,begin, end, terminate, j=0, matchwindow;
      while(j<=((text.length()-patternLength))){
          System.out.println("j="+j);
          pos=1; begin=j; end=begin+c; terminate=0; matchwindow=1;     //กำหนดค่าเริ่มต้น
