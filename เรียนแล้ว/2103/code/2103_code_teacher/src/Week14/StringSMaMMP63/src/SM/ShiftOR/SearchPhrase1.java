 ...
 // String p="aaaa", text="aaaaaaaabcdeabcdaaaabcd";   
  ...
  // int count=0;
   ...
   public SearchPhrase1(String p){
    ...
    for(int i=0;i<65;i++){
        B[i]=~0;
        System.out.println("Bit[:"+i+"]="+Long.toBinaryString(B[i]));
    }
    for(int j=0;j<pattern.length();j++) {
      B[gotoChar(pattern.charAt(j))]= B[gotoChar(pattern.charAt(j))]&(~(1<<j));        
   ...
   public void mainSearch(String text) throws IOException{
       int m=pattern.length();
        long D=~0, mm=1<<(m-1);
        int i=0;
        System.out.println("--- Searching Befor loop ----");
        System.out.println("D:"+Long.toBinaryString(D)+" mm:"+Long.toBinaryString(mm));
        System.out.println("--- Searching In loop ----");
        while(i<text.length()){
            System.out.println("-----ความพยายามครั้งที่ :"+(i+1)+"---------");
            D=(D<<1)|B[gotoChar(text.charAt(i))];
            System.out.println("i:"+i+" T[i]:"+text.charAt(i));
            System.out.println("D:"+Long.toBinaryString(D)+" mm:"+Long.toBinaryString(mm));
            System.out.println("D&mm:"+Long.toBinaryString(D&mm));
            if((D&mm)!= mm){ 
                System.out.println("match at:"+(i+1));
            }
            i++;
            //System.out.println("i:"+i+" text.length:"+text.length());
        }//end while
    }
    //*****************************
