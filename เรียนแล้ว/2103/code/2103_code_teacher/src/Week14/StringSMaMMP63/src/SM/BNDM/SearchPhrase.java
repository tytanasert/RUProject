  ...
  // int count=0;
   ...
   public void mainSearch(String text) throws IOException{
      ...
      while(i<=(text.length()-pattern.length())){
          int j=m-1, last=m-1, D=(1<<m)-1;
          System.out.println(Integer.toBinaryString(D));
        while(D!=0){
            D=D&B[gotoChar(text.charAt((i+j)))];
            //System.out.println("i+j:"+(i+j)+" B:"+(B[gotoChar(text.charAt((i+j)-1))])+" char:"+(text.charAt((i+j)-1)));
            j--;
            timecount++;
            if( (D&(1<<(m-1)))!=0) {
               if (j>0) 
            ...
            D=D<<1;
            System.out.println("Bit D:"+Integer.toBinaryString(D));
        }//end while  
          i+=last;
      }
    ...
    //*****************************
     public static void main(String args[]) throws Exception{
      SearchPhrase a = new SearchPhrase("abcd");
      a.mainSearch("abcdeabcdaaabcd");
  }
}
