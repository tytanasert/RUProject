...
public class QSAlgorithm {
    ...
    public int[] preQsBc(String p, int qsBc[]){
      int m=p.length();  
      int i;
      for(i=0;i<66;++i) qsBc[i]=m+1;
       for(i=0;i<m;++i) qsBc[gotoChar(p.charAt(i))]=m-i; 
       return qsBc;
    }
    ...
    public void QSSearch(String p, String T) {
        ...
        j=0;       
        while(j<=n-m){
            System.out.println("--- ความพยายามครั้งที่ "+(j+1)+" ค่า j="+j);
            System.out.println("T[j…j+m]="+T.substring(j,j+m));
               if(T.substring(j,j+m).compareTo(p)==0) 
                  System.out.println("ค้นพบที่ตำแหน่ง:"+(j+m));
              System.out.println("T[j+m]="+T.charAt(j+m)+"คำนวณค่า j = "+j+"+"+"qsBc["+qsBc[gotoChar(T.charAt(j+m))]+"]"); 
                      j+=qsBc[gotoChar(T.charAt(j+m))];   
          //            System.out.println("คำนวณค่า j = "+j+"+"+"qsBc["+qsBc[gotoChar(T.charAt(j+m))]+"]");
        }
    }
