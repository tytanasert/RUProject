package MMP.HP;
import java.io.*;
public class HPAlgorithm1 {          
   BuildHPTrie1 pattern = new BuildHPTrie1();    
    public void HPSearch(String P[], String text) throws IOException{
        try{
           //Preprocessing
            ...
            int pos=lmin-1;
           while(pos<text.length())   //เริ่มค้นหา
           { 
               int j=0; 
                  Current=(DataNode)RoFunction(0,text.charAt(pos),Current,pos);
               j++;   
               while( ((pos-j)>=0) && Current!=null ){//end while              
                      if(RoFunction(1,text.charAt(pos-j),Current,pos-j)!=null)
                         Current = (DataNode)RoFunction(1,text.charAt(pos-j),Current,pos-j);
                      if(Current !=null) 
                      if(Current.getTerminate()==1) //set Mark founded state
                       {
                           System.out.println("ค้นพบที่ตำแหน่ง :"+(pos+1) +" อักขระแบบ :"+
                                   text.substring(pos-j,pos+1));
                           Current=null;
                       }                           
                        j++;
               }//end while  
               pos = pos+pattern.dTable[caseNode(text.charAt(pos))];
           }//end while    
