...
public class BNDMAlgorithm {
   long B[] = new long[67];
   public long [] preBNDM(String p, long B[]){
    for(int i=0;i<65;i++) {
        B[i]=0;
    }
    for(int j=0;j<p.length();j++) {
      B[gotoChar(p.charAt(j))]=B[gotoChar(p.charAt(j))]|(1<<(p.length()-j));        
    }
    return B;
   }
   ...
   public void BNDMSearch(String p, String T){
      ...
      while(i<=n-m){
          ...
          long D=(1<<m)-1;
        while(D!=0){            
            D=D&B[gotoChar(T.charAt((i+j)))];           
            j--;           
            if( (D&(1<<(m-1)))!=0) {
               if (j>0) 
                  last=j;
               else{
                   System.out.println("ค้นพบที่ตำแหน่ง:"+(i+m));
                   D=0;
               }
            }//end if            
            D=D<<1;
