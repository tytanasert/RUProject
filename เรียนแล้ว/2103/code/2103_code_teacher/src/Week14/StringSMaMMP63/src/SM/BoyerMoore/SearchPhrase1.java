...
//   public char []text;  
   ...
   String pattern;
    ...
    public SearchPhrase1(String str) {
      pattern = str;    
    }
    ...
    public long BM(String x, int m, String y,int n){
        int i,j,bmGs[]=new int[x.length()], bmBc[]=new int[52];
        //preprocessing
        preBmGs(x,m,bmGs);
        preBmBc(x,m,bmBc);
        //***Seraching***
        ...
        while(j<=n-m){
            ...
            for(i=m-1;i>=0&&x.charAt(i)==y.charAt(i+j);--i);//{timecount++;};
                if(i<0){
                    ...
                    System.out.println("j Befor set after Match:j="+j+" i="+i);
                    j+=bmGs[0];
                    System.out.println("set new window search :"+j+"bmGs[0]:"+bmGs[0]);
                }
                else{
                    System.out.println("Befor:calculate: new window,\nbmGs[i],bmBc[gotoChar(y.charAt(i+j))])-m+1+i:\n"+
                            bmGs[i]+","+bmBc[gotoChar(y.charAt(i+j))]+"-"+m+"+1+"+i+" j="+j+" i="+i);
                    j+=MAX(bmGs[i],bmBc[gotoChar(y.charAt(i+j))])-m+1+i;
                    System.out.println("set new window search :"+j);
                   // timecount++;
                }
        }//end while
